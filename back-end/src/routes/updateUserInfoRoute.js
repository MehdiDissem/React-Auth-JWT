...
export const updateUserInfoRoute = {
    ...
    handler: async (req,res)=>{
        ...
        jwt.verify(token, process.env.JWT_SECRET, async(err,decoded)=>{
            ...
            if (id !== userId) return res.status(403).json({message:'Not allowed to update user'})

            if (!isVerified) return res.status(403).json({message:'you need to verify your email before you update your data'})
